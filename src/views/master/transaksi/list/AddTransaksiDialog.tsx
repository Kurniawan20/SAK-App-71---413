...
const AddTransaksiDialog = ({ open, onClose, onSubmit, lastId }: AddTransaksiDialogProps) => {
  // States
  ...
  const handleTextFieldChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name) {
      ...
      if (['pokok_pembiayaan', 'margin', 'tenor_bulan'].includes(name)) {
  ...
  const handleSelectChange = (e: SelectChangeEvent) => {
    ...
    if (name) {
      // Handle special case for numeric fields
      if (['nasabah_id', 'akad_id', 'kolektibilitas_terkini'].includes(name)) {
        const numValue = Number(value)
        setFormData(prev => ({ 
          ...prev, 
          [name]: isNaN(numValue) ? 0 : numValue 
      ...
      <DialogContent dividers sx={{ p: theme => theme.spacing(4) }}>
        <Grid container spacing={4}>
          ...
          <Grid item xs={12}>
            <TextField
              fullWidth
              label='Tanggal Update Kolektibilitas'
              name='tanggal_update_kolektibilitas'
              type='date'
              value={formData.tanggal_update_kolektibilitas}
              onChange={handleTextFieldChange}
              InputLabelProps={{ shrink: true }}
            />
          </Grid>
...
export default AddTransaksiDialog
